...
You can upload manually via the **GitHub website** or use **Git** (recommended for updates later).

### Method 1 — GitHub website (simple, one-time upload)

1. Create a free account at [https://github.com](https://github.com).
2. Click **+ → New repository**.
3. Set repository name: `WorkOps` (or your preferred name).
...
8. Drag and drop your project folder contents:
   - Include: `WorkOps/`, `Database.sql`, `README.md`, `.gitignore`, `WorkOps.slnx`, `WorkOps Images/`, `Workops Documents/`, `packages/` (if present)
   - **Exclude:** `.vs/`, `WorkOps/bin/`, `WorkOps/obj/` (these are in `.gitignore`)
9. Scroll down, enter commit message: `Initial commit - WorkOps project`
10. Click **Commit changes**.

> GitHub web upload has a **25 MB per file** limit. PDFs and images should be fine; avoid uploading large zip files of `bin/`.

### Method 2 — Git command line (recommended)

Open **PowerShell** or **Git Bash** in your project folder (`e:\Diploma Project\WorkOps`):

```powershell
# 1. Initialize Git (once)
git init

# 2. Stage all files (.gitignore excludes bin, obj, .vs automatically)
git add .

# 3. First commit
git commit -m "Initial commit - WorkOps Smart Company Management System"

# 4. Create repo on GitHub (via website), then link it — replace YOUR_USERNAME
git remote add origin https://github.com/YOUR_USERNAME/WorkOps.git
...
If Git asks you to sign in, use a **Personal Access Token** (PAT) instead of your password:  
**GitHub → Settings → Developer settings → Personal access tokens → Generate new token**

### Method 3 — GitHub Desktop

1. Install [GitHub Desktop](https://desktop.github.com/).
2. **File → Add local repository** → select your `WorkOps` folder.
3. If prompted to create a repository, accept.
4. Write commit summary → **Commit to main**.
5. **Publish repository** → choose name and visibility → **Publish**.

---

## Updating GitHub After New Changes

When you add features or fix bugs locally, push the changes so GitHub stays in sync.

### Using Git commands (after initial setup)

```powershell
cd "e:\Diploma Project\WorkOps"
...
git commit -m "Add bulk attendance export feature"
...
2. Enter a **Summary** (commit message).
3. Click **Commit to main**.
4. Click **Push origin**.

### Good commit message examples

- `Fix login validation for special characters in password`
